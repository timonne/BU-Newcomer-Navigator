...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      strictPort: true,
      // ADDED FOR BACKEND INTEGRATION — the only change to this file.
      // Proxies /api to the FastAPI backend so the frontend and API share an
      // origin. That is what lets the backend's httpOnly auth cookie work:
      // cross-origin it would need SameSite=None + Secure, which plain
      // http://localhost cannot provide. Override the target with
